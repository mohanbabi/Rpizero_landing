...
def send_land_message(x, y, z):
    # Check if the marker is fully visible
    if 0 < x < horizontal_resolution and 0 < y < vertical_resolution:
...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    if ids is not None and 72 in ids:
        ...
        # Adjust x and y if the marker is not fully visible
        if x < 0 or x > horizontal_resolution:
            x = horizontal_resolution // 2
        if y < 0 or y > vertical_resolution:
            y = vertical_resolution // 2

        # Send the adjusted landing message
