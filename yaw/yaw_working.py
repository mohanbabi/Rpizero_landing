...
def yaw(desired_yaw_radians):
    quaternion = [math.cos(desired_yaw_radians / 2), 0, 0, math.sin(desired_yaw_radians / 2)]
    msg=vehicle.message_factory.set_attitude_target_send(
        0,  # time_boot_ms
        10,
        0,
...
while True:
    ...
    if vehicle.mode in [VehicleMode("RTL"), VehicleMode("LAND")] or vertical_velocity > 0.09:
        if (vehicle.armed):
            yaw(desired_yaw_radians)
            time.sleep(0.2)
